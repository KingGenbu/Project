...
//        UserDefaultHelper.setArrPREF(self.arrSeenNotification, key: "pref_notification_seen")
        ...
        if notificationData.notificationType == NotificationType.typeIsLive.rawValue {
            ...
            let removeHydroXFriindVC = Helper.mainStoryBoard.instantiateViewController(withIdentifier: StoryboardIdentefier.removeHydroXFriend.rawValue) as! RemoveHydroXFreindsVC
            removeHydroXFriindVC.isFromNotification = true
            Helper.Push_Pop_to_ViewController(destinationVC: removeHydroXFriindVC, isAnimated: true, navigationController: self.navigationController!)
        } else if notificationData.notificationType == NotificationType.typeStory.rawValue {
