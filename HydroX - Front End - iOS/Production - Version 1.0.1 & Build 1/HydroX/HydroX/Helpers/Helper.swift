...
class Helper : NSObject, MFMailComposeViewControllerDelegate {
            ...
            if error == nil {
                if let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 200 {
                    if let dict = json.dictionaryObject {
                        UserDefaultHelper.setDicPREF(dict, key: AppUserDefaults.pref_dictProfile)
                    }
                }
            } else {
    ...
    class func WSLogoutCalled(navigation: UINavigationController) {
        ...
        Helper.showAlertDialogWith2Button(onVC: vc, title: APP_NAME, message: AppMessage.logoutMessage.rawValue, button1Title: "Cancel", button1ActionStyle: .cancel, button2Title: "Logout", onButton1Click: nil) {
...
extension MFMailComposeViewController: MFMailComposeViewControllerDelegate {
    func showMailComposer(navigation: UINavigationController) {
        if MFMailComposeViewController.canSendMail() {
            let strVersionNumber = Bundle.main.object(forInfoDictionaryKey: "CFBundleShortVersionString") as? String ?? "Unknown"
            let strBuildNumber = Bundle.main.object(forInfoDictionaryKey: "CFBundleVersion") as? String ?? "Unknown"
