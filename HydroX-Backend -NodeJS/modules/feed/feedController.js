...
feedCtr.goLiveGetStreamId = (req, res) => {
  ...
  let { streamToHydroX } = req.body || false;
  ...
  streamToHydroX = streamToHydroX === '1';
            ...
            if (sum === constants.maxLit) {
              ...
              Device.find({
                ...
                .then((deviceDocs) => {
                  ...
                  // Send Notification
                  // logger.info(feed.user._id);
                  notificationUtils.sendNotification(feed.user._id, devices, 'HydroXDone', { feed: feedId, userId: req.user._id });
                });
            }
            // Update count in cache
          ...
          if (sum === constants.maxLit) {
            ...
            Device.find({
              ...
              .then((deviceDocs) => {
                ...
                notificationUtils.sendNotification(data[0].user, devices, 'HydroXDone', { feed: feedId, userId: req.user._id });
              });
          }
          // Update count in cache
