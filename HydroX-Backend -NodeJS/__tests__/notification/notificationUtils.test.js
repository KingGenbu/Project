...
describe('notificationUtils.NoteType', () => {
  it('exports expected notification type keys', () => {
    ...
    expect(notificationUtils.NoteType.HydroXDone).toBe('HydroXDone');
  });
});
...
describe('notificationUtils.sendBulkNotification', () => {
  ...
  it('resolves HydroXDone message without a DB lookup', async () => {
    notificationUtils.sendBulkNotification(
      [FAKE_USER_ID],
      ['device-token-1'],
      notificationUtils.NoteType.HydroXDone,
      {},
    );
